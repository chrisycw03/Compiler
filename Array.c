...
void ArrayAdd(Array *array, void *item){								// add item to array
	ASSERT(array->count <= array->size);
	if(array->count == array->size){									// if count == size, double array size
	...
	array->item[array->count++] = item;									// assign item to array->item
	// printf("add item = %s\n", item);
}
